...
$mang = [2, 9, 5, 7, 1, 4]; // Thay đổi giá trị của mảng ở đây để tìm giá trị lớn nhất và nhỏ nhất khác
...
<?php
function incrementalValue($mang) {
    $n = count($mang);
...
$mang = [5, 3, 9, 1, 7]; // Thay đổi giá trị của mảng ở đây để sắp xếp các số khác

incrementalValue($mang);
?>
<?php
function arrangesort($array) {
    $n = count($array);
    for ($i = 0; $i < $n - 1; $i++) {
        for ($j = $i + 1; $j < $n; $j++) {
            if ($array[$i] > $array[$j]) {
...
timSoNguyenToTrongKhoang($start, $end);
?>
